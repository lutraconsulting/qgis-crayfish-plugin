...
struct Node
{
    ...
    const char* toWkt() const {return QString("POINT(%1,%2)").arg(x, y).toLatin1().data(); }
...
struct BBox
{
  ...
  const char* toWkt() const {return QString("POLYGON(%1,%2,%3,%4)").arg(minX,maxX, minY, maxY).toLatin1().data(); }
};
